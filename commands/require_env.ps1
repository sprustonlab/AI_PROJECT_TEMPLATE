...
$script:PLATFORM_SUBDIR = "win-64"
$script:PLATFORM_ENVS = "$SLC_DIR\envs\$PLATFORM_SUBDIR"
...
$condaHook = "$PLATFORM_ENVS\SLCenv\shell\condabin\conda-hook.ps1"
...
#
function Test-SlcInstalled {
    $condaExe = "$PLATFORM_ENVS\SLCenv\Scripts\conda.exe"
...
function Test-EnvInstalled {
    param([string]$Name)
    return (Test-Path "$PLATFORM_ENVS\$Name")
}
...
function Install-Env {
    ...
    $env:SLC_PYTHON = "$PLATFORM_ENVS\SLCenv\python.exe"
    $env:PYTHONPATH = "$SLC_DIR\modules;$env:PYTHONPATH"
    $env:CONDA_ENVS_PATH = "$PLATFORM_ENVS;$env:CONDA_ENVS_PATH"
    $env:PATH = "$PLATFORM_ENVS\SLCenv;$PLATFORM_ENVS\SLCenv\Scripts;$env:PATH"
...
if ($EnvName) {
    ...
    conda activate "$PLATFORM_ENVS\$EnvName"
    if ($LASTEXITCODE -ne 0) {
