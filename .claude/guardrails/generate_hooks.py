...
TRIGGER_TO_FILE = {
    "PreToolUse/Bash": "bash_guard.sh",
    "PreToolUse/Read": "read_guard.sh",
    "PreToolUse/Glob": "glob_guard.sh",
    "PreToolUse/Write": "write_guard.sh",
    "PreToolUse/Edit": "write_guard.sh",   # Write and Edit share a hook script
    "SessionStart/compact": "post_compact_injector.sh",
}
...
def generate_bash_guard(rules: list[dict], catalog_version: str, ack_ttl: int = 60) -> str:
    """Generate bash_guard.sh content for PreToolUse/Bash rules.
    ...
    Returns:
        Generated bash_guard.sh content string.
    """
    ...
    lines.append(f"# bash_guard.sh")
    lines.append(f"# AUTO-GENERATED by .claude/guardrails/generate_hooks.py from rules.yaml")
    lines.append(f"# DO NOT EDIT DIRECTLY — edit rules.yaml and re-run:")
    lines.append(f"#   python3 .claude/guardrails/generate_hooks.py")
    ...
    lines.append('GUARDRAILS_DIR="$(dirname "${SCRIPT_DIR}")"')
    lines.append('HITS_FILE="${GUARDRAILS_DIR}/hits.jsonl"')
    lines.append("")
    lines.append("INPUT=$(cat)")
    lines.append('export HOOK_INPUT="${INPUT}"')
    lines.append("export HITS_FILE")
    lines.append("export GUARDRAILS_DIR")
    lines.append("")
    ...
    lines.append("")
    lines.append("data = json.loads(os.environ.get('HOOK_INPUT', '{}'))")
    lines.append("hits_file = os.environ.get('HITS_FILE', '')")
    ...
    if has_role_guard:
        ...
        lines.append("sys.path.insert(0, os.environ.get('GUARDRAILS_DIR', '.claude/guardrails'))")
...
def generate_read_guard(rules: list[dict], catalog_version: str) -> str:
    """Generate read_guard.sh content for PreToolUse/Read rules.

    Uses collect-then-dispatch pattern: rules append to _matched_rules, then
    the dispatch block handles deny > warn > inject > log priority.
    ...
    Returns:
        Generated read_guard.sh content string.
    """
    rule_ids = ", ".join(r["id"] for r in rules)
    lines = []
    lines.append("#!/usr/bin/env bash")
    lines.append("# read_guard.sh")
    lines.append("# AUTO-GENERATED by .claude/guardrails/generate_hooks.py from rules.yaml")
    lines.append("# DO NOT EDIT DIRECTLY — edit rules.yaml and re-run:")
    lines.append("#   python3 .claude/guardrails/generate_hooks.py")
    lines.append(f"# catalog_version: {catalog_version}")
    lines.append(f"# Rules: {rule_ids}")
    lines.append("")
    lines.append('SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"')
...
def generate_glob_guard(rules: list[dict], catalog_version: str) -> str:
    """Generate glob_guard.sh content for PreToolUse/Glob rules.
    ...
    Returns:
        Generated glob_guard.sh content string.
    """
    rule_ids = ", ".join(r["id"] for r in rules)
    lines = []
    lines.append("#!/usr/bin/env bash")
    lines.append("# glob_guard.sh")
    lines.append("# AUTO-GENERATED by .claude/guardrails/generate_hooks.py from rules.yaml")
    lines.append("# DO NOT EDIT DIRECTLY — edit rules.yaml and re-run:")
    lines.append("#   python3 .claude/guardrails/generate_hooks.py")
    lines.append(f"# catalog_version: {catalog_version}")
...
def generate_write_guard(write_rules: list[dict], edit_rules: list[dict],
    ...
    """Generate write_guard.sh content for PreToolUse/Write and PreToolUse/Edit rules.
    ...
    Returns:
        Generated write_guard.sh content string.
    """
...
def generate_post_compact_injector(rules: list[dict], catalog_version: str) -> str:
    """Generate post_compact_injector.sh for SessionStart/compact rules."""
    rule_ids = ", ".join(r["id"] for r in rules)
    lines = []
    lines.append("#!/usr/bin/env bash")
    lines.append("# post_compact_injector.sh")
    lines.append("# AUTO-GENERATED by .claude/guardrails/generate_hooks.py from rules.yaml")
    lines.append("# DO NOT EDIT DIRECTLY — edit rules.yaml and re-run:")
    lines.append("#   python3 .claude/guardrails/generate_hooks.py")
    lines.append(f"# catalog_version: {catalog_version}")
    ...
    lines.append("INPUT=$(cat)")
    lines.append("")
    lines.append('SESSION_ID=$(echo "${INPUT}" | python3 -c "')
    lines.append("import json, sys")
    lines.append("try:")
    lines.append("    d = json.load(sys.stdin)")
    lines.append("    print(d.get('session_id', 'unknown'))")
    lines.append("except Exception:")
    lines.append("    print('unknown')")
    lines.append('" 2>/dev/null || echo "unknown")')
    lines.append("")
    lines.append("TS=$(python3 -c \"from datetime import datetime,timezone; print(datetime.now(timezone.utc).strftime('%Y-%m-%dT%H:%M:%SZ'))\")")
    lines.append("")

    # Log hits for each rule
    for rule in rules:
        rule_id = rule["id"]
        lines.append(f'echo "{{\\"ts\\":\\"${{TS}}\\",\\"rule_id\\":\\"{rule_id}\\",\\"session_id\\":\\"${{SESSION_ID}}\\",\\"enforcement\\":\\"inject\\",\\"tool\\":\\"SessionStart\\",\\"snippet\\":\\"compact\\"}}" >> "${{HITS_FILE}}" 2>/dev/null || true')

    lines.append("")

    # Emit message content
    lines.append("cat <<'INJECTION'")
    for rule in rules:
        message_text = get_message_text(rule)
...
def trigger_to_hook_filename(trigger: str) -> str:
    ...
    Example: 'mcp__chic__spawn_agent' → 'mcp__chic__spawn_agent_guard.sh'
...
def update_settings_json(new_triggers: list[str]) -> None:
    ...
    for trigger in new_triggers:
        hook_file = f".claude/guardrails/hooks/{trigger_to_hook_filename(trigger)}"
        existing = next((e for e in pre_tool_use if e.get('matcher') == trigger), None)
        if existing:
            current_cmd = (existing.get('hooks') or [{}])[0].get('command', '')
            if current_cmd != hook_file:
                existing['hooks'] = [{'type': 'command', 'command': hook_file}]
                changed = True
        else:
            pre_tool_use.append({
                'matcher': trigger,
...
def generate_all(output_dir: Path) -> dict[str, str]:
    ...
    if bash_rules:
        generated["bash_guard.sh"] = generate_bash_guard(bash_rules, catalog_version, ack_ttl)

    # Read guard
    read_rules = groups.get("PreToolUse/Read", [])
    if read_rules:
        generated["read_guard.sh"] = generate_read_guard(read_rules, catalog_version)

    # Glob guard
    glob_rules = groups.get("PreToolUse/Glob", [])
    if glob_rules:
        generated["glob_guard.sh"] = generate_glob_guard(glob_rules, catalog_version)

    # Write/Edit guard (merged into write_guard.sh)
    write_rules = groups.get("PreToolUse/Write", [])
    edit_rules = groups.get("PreToolUse/Edit", [])
    if write_rules or edit_rules:
        generated["write_guard.sh"] = generate_write_guard(
    ...
    if compact_rules:
        generated["post_compact_injector.sh"] = generate_post_compact_injector(
